...
def getitem(idx,leading):
    ...
    return lr,hr
        '''
    en= ['e01', 'e02', 'e03', 'e04', 'e05', 'e06', 'e07', 'e08', 'e09']
    lr=[]
    # read_data filemame[idx]
    for i in range(9):
        filename = "//scratch/iu60/rh4668/test_data/2012/" + en[i] + "/da_pr_" + idx.strftime("%Y%m%d") + "_" + en[i] + ".nc"
        ...
        if i==0:
            ...
            new_e=np.concatenate((lr[i-1], var), axis=-1)
            lr.append(new_e)
    
    lr_transforms = transforms.Compose([transforms.ToTensor()])

    return lr_transforms(lr[8])
...
def test(batch_input):
    ...
    #out=torch.nn.functional.interpolate (slice_output,size=(691,886),mode='bicubic',align_corners=True)
...
model_path=r"/scratch/iu60/rh4668/VDSR_crps/train/save/VDSR_crpsmodel_epoch_27.pth"
model=torch.load(model_path,map_location=device)['model']

with torch.no_grad():
    rootdir="/scratch/iu60/rh4668/test_data/2012/"
    exx = ['e01', 'e02', 'e03', 'e04', 'e05', 'e06', 'e07', 'e08', 'e09']
    start_date=date(2012,1,1)
    end_date=date(2012,12,31)
    files=get_time_without_en(rootdir,start_date,end_date)
    files.sort()
    for t in files:
        fn=rootdir  +"e01/da_pr_" + t.strftime("%Y%m%d") + "_e01.nc"
        ...
        da_interp = xr.DataArray(np.expm1(batch_output * 7), dims=("en","lat", "lon"),coords={"en":exx, "lat": new_lat, "lon": new_lon, "time": i}, name='pr')
        ...
        for i in ds_raw['time'].values[1:30]:
            ...
            batch_output=test(batch_input)
            da_interp = xr.DataArray(np.expm1(batch_output * 7), dims=("en","lat", "lon"),coords={"lat": new_lat, "lon": new_lon, "time": i}, name='pr')
            expanded_da=xr.concat([da_interp],"time")
            ds_total=xr.merge([ds_total,expanded_da])

        save_path="/scratch/iu60/rh4668/VDSR_crps/eval/2012/" + t.strftime("%Y%m%d")  + ".nc"
