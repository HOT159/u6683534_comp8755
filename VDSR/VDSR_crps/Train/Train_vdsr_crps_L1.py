...
from Loss.L1Loss import L1_loss
import torch
from datetime import timedelta, date, datetime
...
class param_args():
    ...
    '''
    def __init__(self):
        self.train_name   ='VDSR_crps_L1'
        self.resume     =''#module path
        self.test       =False
        self.test_model_name="RFDNX4X2"
    ...
    def main(self):
        ...
        for epoch in range(start_epoch, self.args.nEpochs + 1):
            ...
            for iteration, (pr,hr,_,_) in enumerate(train_dataloders):
                ...
                #print(out.shape)
                loss1 = criterion(out,hr)
                loss2= criterion1(out,hr)
                loss=0.3*loss1+0.8*loss2
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                avg_loss+=loss.item()
