...
class Access_AWAP_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        e9=read_access_data(self.file_ACCESS_dir, en, access_date, time_leading, "pr")
        for i in range(8):
            tmp=read_access_data(self.file_ACCESS_dir, self.ensemble[i], access_date, time_leading, "pr")
            if i==0:
                lr.append(tmp)
            else:
                new_e=np.concatenate((lr[i-1], tmp), axis=-1)
                lr.append(new_e)
        all=np.concatenate((lr[i], e9), axis=-1)
        lr.append(all)
        hr = read_awap_data(self.file_AWAP_dir, awap_date)

        return self.lr_transform(lr[8]),self.hr_transform(hr), awap_date.strftime("%Y%m%d"), time_leading
